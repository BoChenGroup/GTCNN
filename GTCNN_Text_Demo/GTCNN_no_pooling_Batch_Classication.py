...
for it in range(Setting.Iter):
    ...
    if Setting.T > 1:
        ...
        Params.p_n[1] = cupy.random.beta(p2_n_aug + SuperParams.a0, Phi_Theta_flat + SuperParams.b0).get()

    else:
        p2_n_aug = np.sum(np.sum(np.sum(W_nk_Aug[0], axis=3), axis=2), axis=1)
        Params.p_n[1] = cupy.random.beta(p2_n_aug + SuperParams.a0, np.sum(Params.Gamma) + SuperParams.b0).get()
    ...
    for t in [i for i in range(Setting.T + 1) if i > 1]:  # only T>=2 works  ==> for t = 2:T
...
for it in range(Setting.Iter):
    ...
    if Setting.T > 1:
        p2_n_aug = np.sum(np.sum(np.sum(W_nk_Aug[0], axis=3), axis=2), axis=1)
        Phi_Theta_flat = np.sum(np.sum(np.sum(Phi_Theta[0], axis=3), axis=2), axis=1)  # conv(Phi_2, Theta_2)
        Params.p_n_te[1] = cupy.random.beta(p2_n_aug + SuperParams.a0, Phi_Theta_flat + SuperParams.b0).get()

    else:
        p2_n_aug = np.sum(np.sum(np.sum(W_nk_Aug[0], axis=3), axis=2), axis=1)
        Params.p_n_te[1] = cupy.random.beta(p2_n_aug + SuperParams.a0, np.sum(Params.Gamma) + SuperParams.b0).get()
    Params.c_n_te[1] = (1 - Params.p_n_te[1]) / Params.p_n_te[1]

    for t in [i for i in range(Setting.T + 1) if i > 1]:  # only T>=2 works  ==> for t = 3:T+1
        if t == Setting.T:
            Params.c_n_te[t] = cupy.random.gamma(shape=np.sum(Params.Gamma) + SuperParams.e0, scale=1).get()
    ...
    for t in range(Setting.T - 1, -1, -1):  ## for t = T:-1 :1
        if t == Setting.T - 1:
            # 更新最高层的Theta
            # N*K_t-1*K_T*K_S1*K_S2
            Params.W_nk_te[t] = cupy.random.gamma(W_nk_Aug[t] + Params.Gamma[np.newaxis, :], scale=1).get()  # N*K_T*K_S1*K_S2
...
clf = svm.SVC()                             # class
W_train = (Params.W_nk_collect[0]) #- np.mean(Params.W_nk_collect[0], axis=1, keepdims=True)) / np.std(Params.W_nk_collect[0], axis=1, keepdims=True)  # np.sum(np.sum(Params['W_nk_collect'][0]/Setting['Collection'], axis=3), axis=2)
W_test = (Params.W_nk_te_collect[0]) #- np.mean(Params.W_nk_te_collect[0], axis=1, keepdims=True)) / np.std(Params.W_nk_te_collect[0], axis=1, keepdims=True)  # np.sum(np.sum(Params['W_nk_te_collect'][0]/Setting['Collection'], axis=3), axis=2)

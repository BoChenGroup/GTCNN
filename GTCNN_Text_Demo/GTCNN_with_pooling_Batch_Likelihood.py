...
count_value = 25  # we use this to simplify binary link
...
Setting.K = [200, 100]  # convlutional layer settings
Setting.K_Phi = [50]  # fully connected layer settings
...
# initial convolutional generative model
for t in range(Setting.T):
...
for it in range(Setting.Iter):
    ...
    #         Params.c_n_Phi[t_phi] = np.random.gamma(SuperParams.e0 + np.sum(Params.Gamma_Phi, axis=0))
    #     else:
    #         Params.c_n_Phi[t_phi] = np.random.gamma(SuperParams.e0 + np.sum(Params.Theta[t_phi+1], axis=0))
    #
    ...
    # update Theta
    for t_phi in range(Setting.T_Phi - 1, -1, -1):
        if t_phi == Setting.T_Phi - 1:
...
sio.savemat(file_name + '_PGCBN_MR.mat', {'Likelihood': Likelihood_list,
